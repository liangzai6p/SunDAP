...
public class ArmsDataStatisticsController extends BaseController {
    ...
    @RequestMapping(value = "/armsDataStatisticsController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    @ArchivesLog(operationType="增删改查", operationName="模型统计配置")
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        return super.excuteRequest(request, ArmsDataStatistics.class);
    }

    @Override
    protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
        try {
            responseBean = armsDataStatisticsService.execute(requestBean);
        } catch (Exception e) {
