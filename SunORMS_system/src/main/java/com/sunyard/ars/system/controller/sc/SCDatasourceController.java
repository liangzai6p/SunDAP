...
@Controller
public class SCDatasourceController extends BaseController {
	
	@Resource
	private ISCDatasourceService scDatasourceService;
	
	@RequestMapping(value = "/scDatasourceController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作系统来源定义表")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, SCDatasource.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = scDatasourceService.execute(requestBean);
		} catch (Exception e) {
