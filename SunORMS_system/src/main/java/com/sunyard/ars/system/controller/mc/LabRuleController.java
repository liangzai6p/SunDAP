...
@Controller
public class LabRuleController extends BaseController {

	
	@Resource
	private ILabRuleService labRuleService;
	
	@RequestMapping(value = "/labRuleController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="实验室规则配置")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, LabRule.class);
	...
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = labRuleService.execute(requestBean);
		} catch (Exception e) {
