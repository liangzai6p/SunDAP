...
@Controller
public class ServiceRegController extends BaseController {
	
	@Resource
	private IServiceRegService serviceRegService;
	
	@RequestMapping(value = "/serviceRegController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作注册服务信息")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, ServiceReg.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = serviceRegService.execute(requestBean);
		} catch (Exception e) {
