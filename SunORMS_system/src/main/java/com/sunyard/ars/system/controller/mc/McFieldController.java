...
@Controller
public class McFieldController extends BaseController {

	
	@Resource
	private IMcFieldService mcFiledService;
	
	@RequestMapping(value = "/mcFieldController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="字段配置")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, McField.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = mcFiledService.execute(requestBean);
		} catch (Exception e) {
