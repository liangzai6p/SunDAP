...
public class VoucherInfoController extends BaseController {
	...
	@RequestMapping(value = "/voucherInfoController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作凭证定义表")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, VoucherInfo.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = voucherInfoService.execute(requestBean);
		} catch (Exception e) {
