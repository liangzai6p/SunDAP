...
@Controller
public class organModelController extends BaseController {

	
	@Resource
	private IOrganModelService organModelService;
	
	@RequestMapping(value = "/organModelController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="机构模型")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, OrganModel.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = organModelService.execute(requestBean);
		} catch (Exception e) {
