...
@Controller
public class ExhibitFieldController extends BaseController {

	
	@Resource
	private IExhibitFieldService exhibitFieldService;
	
	@RequestMapping(value = "/exhibitFieldController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="展现字段配置")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, ExhibitField.class);
	...
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = exhibitFieldService.execute(requestBean);
		} catch (Exception e) {
