...
@Controller
public class SmDayoffTbController extends BaseController {
	
	@Resource
	private ISmDayoffTbService ISmDayoffTbService ;
	
	@RequestMapping(value = "/smDayoffTbController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="节假日表")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, SmDayoffTb.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = ISmDayoffTbService.execute(requestBean);
		} catch (Exception e) {
