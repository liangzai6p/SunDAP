...
@Controller
public class SlipDefController extends BaseController {
	
	@Resource
	private ISlipDefService slipDefService;
	
	@RequestMapping(value = "/slipDefController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作差错定义表")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, SlipDef.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = slipDefService.execute(requestBean);
		} catch (Exception e) {
