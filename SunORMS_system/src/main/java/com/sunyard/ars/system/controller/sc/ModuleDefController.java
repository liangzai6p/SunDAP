...
@Controller
public class ModuleDefController extends BaseController {
	
	@Resource
	private IModuleDefService moduleDefService;
	
	@RequestMapping(value = "/moduleDefController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作模块定义表")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, ModuleDef.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = moduleDefService.execute(requestBean);
		} catch (Exception e) {
