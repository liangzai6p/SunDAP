...
@Controller
public class McDataSourceController extends BaseController {

	
	@Resource
	private IMcDataSourceService dataSourceService;
	
	@RequestMapping(value = "/mcDataSourceController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="数据源配置")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, DataSource.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = dataSourceService.execute(requestBean);
		} catch (Exception e) {
