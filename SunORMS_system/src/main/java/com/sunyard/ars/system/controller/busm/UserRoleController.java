...
/**
 * @author:		zheng.jw
 * @date:		2017年12月19日 上午9:24:35
 * @Description:用户权限角色的配置Controller
 */
@Controller
public class UserRoleController extends BaseController {
	 ...
	 * @Description:执行操作请
	 */
	@RequestMapping(value = "/userRoleController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="操作用户角色配置信息")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 执行请求，附带请求参数Bean类型
		return super.excuteRequest(request, UserRole.class);
	}
	...
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = userRoleService.execute(requestBean);
		} catch (Exception e) {
