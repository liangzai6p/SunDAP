...
public class OcrFieldController extends BaseController {
	...
	@Resource
	private IOcrFieldService ocrFieldService;
	
	@RequestMapping(value = "/ocrFieldController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="展现字段配置")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, OcrField.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = ocrFieldService.execute(requestBean);
		} catch (Exception e) {
