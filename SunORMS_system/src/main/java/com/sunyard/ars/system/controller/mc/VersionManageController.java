...
public class VersionManageController extends BaseController {
	...
	@Resource
	private IVersionManageService versionManage;
	
	@RequestMapping(value = "/versionManageController.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="查", operationName="版本信息查看")
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		return super.excuteRequest(request, LabMcVersion.class);
	}
	
	@Override
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = versionManage.execute(requestBean);
		} catch (Exception e) {
