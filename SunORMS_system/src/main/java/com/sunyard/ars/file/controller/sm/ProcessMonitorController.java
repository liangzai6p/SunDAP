...
public class ProcessMonitorController extends BaseController {
	...
	@Resource
	public IProcessMonitorService processMonitorService;
	/**
	 * 
	 * @param request
	 * @param response
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/processMonitor.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="业务进度监控")
	...
	protected ResponseBean doAction(HttpServletRequest req, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = processMonitorService.execute(requestBean);
		} catch (Exception e) {
