...
public class ScanBatchController extends BaseController {
	...
	@Resource
	public IScanBatchService scanBatchService;
	/**
	 * 
	 * @param request
	 * @param response
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/scanBatch.do", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@ResponseBody
	@ArchivesLog(operationType="增删改查", operationName="批次扫描")
	...
	protected ResponseBean doAction(HttpServletRequest req, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = scanBatchService.execute(requestBean);
		} catch (Exception e) {
