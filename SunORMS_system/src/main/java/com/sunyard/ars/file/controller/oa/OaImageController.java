 ...
 */
@Controller
public class OaImageController extends BaseController {

	// 服务接口
	@Resource
	private IOaImageService oaImageService;
	...
	protected ResponseBean doAction(HttpServletRequest request, RequestBean requestBean, ResponseBean responseBean) {
		try {			
			responseBean = oaImageService.execute(requestBean);
		} catch (Exception e) {
